...
@section('content')
    <div class="d-flex mx-4">
        <div class="d-flex align-items-center col-6 ">
            <h1><span class="badge badge-dark text-white display-1 shadow my-4">Server Data</span></h1>
            <div class="ml-5">
                <h5 class="card-text">Created:{{$server->created_at->diffForHumans()}}</h5>
                <h5 class="card-text">Updated:{{$server->updated_at->diffForHumans()}}</h5>
            </div>
        </div>
        ...
        <div class="card shadow my-4 col-6">
            <div class="card-body  pt-0">
                ...
                <p class="card-text">{{$server->type->name}}</p>
            </div>
        </div>
    </div>
        ...
        <div class="d-flex flex-column col-8">
            ...
            <div class="card shadow my-3">
                ...
                <a class="text-white text-decoration-none" href="">Comment:</a>
                    ...
                    <p class="card-text">{{$server->comment}}</p>
                </div>
            </div>
            <div class="card shadow my-3">
                <div class="card-body  pt-0">
                    <div class="d-flex justify-content-between">
                        ...
                        <a href="{{route('server.edit', $server->id)}}"><i class="fas fa-edit text-dark m-3"></i></a>
                    </div>
                    <p class="card-text">{!! $server->notes !!}</p>
                </div>
            </div>
        </div>
                ...
                <div class="d-flex flex-column">
                    @if($photos)
                        @foreach($photos as $photo)
                            <div class="d-flex flex-column">
                                <div class="d-flex">
                                <img class="rounded m-2" height="200" width="200" src="{{$photo ? asset('images/server') . $photo->file : 'http://placehold.it/62x62'}}" alt="{{$server->type}}">
                                {!! Form::open(['method'=>'DELETE',
                                   'action'=>['App\Http\Controllers\ServerController@destroyPhoto', $photo->id]]) !!}
                                ...
                                <div class="modal fade" id="exampleModal{{ $photo->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class=" modal-dialog modal-xl">
                                        <div class="modal-content">
                                            ...
                                            <img class="rounded m-2"  src="{{$photo ? asset('images/server') . $photo->file : 'http://placehold.it/62x62'}}" alt="{{$server->type}}">
                                        </div>
                                    </div>
                                </div>
                            </div>
