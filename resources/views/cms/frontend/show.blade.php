...
@section('content')
    <div class="d-flex mx-4">
        <div class="d-flex align-items-center col-6 ">
            <h1><span class="badge badge-dark display-1 shadow my-4">Frontend Data</span></h1>
            <div class="ml-5">
                <h5 class="card-text">Created:{{$frontend->created_at->diffForHumans()}}</h5>
                <h5 class="card-text">Updated:{{$frontend->updated_at->diffForHumans()}}</h5>
            </div>
        </div>
        ...
        <div class="card shadow my-4 col-6">
            <div class="card-body  pt-0">
                ...
                <p class="card-text">{{$frontend->type->name}}</p>
            </div>
        </div>
    </div>
        ...
        <div class="d-flex flex-column col-8">
            ...
            <div class="card shadow my-3">
                ...
                <a class="text-white text-decoration-none" href="">Comment:</a>
                    ...
                    <p class="card-text">{{$frontend->comment}}</p>
                </div>
            </div>
            <div class="card shadow my-3">
                <div class="card-body  pt-0">
                    <div class="d-flex justify-content-between">
                        ...
                        <a href="{{route('frontend.edit', $frontend->id)}}"><i class="fas fa-edit text-dark m-3"></i></a>
                    </div>
                    <p class="card-text">{!! $frontend->notes !!}</p>
                </div>
            </div>
        </div>
                ...
                <div class="d-flex flex-column">
                    @if($photos)
                        @foreach($photos as $photo)
                            <div class="d-flex flex-column">
                                <div class="d-flex">
                                <img class="rounded m-2" height="200" width="200" src="{{$photo ? asset('images/frontend') . $photo->file : 'http://placehold.it/62x62'}}" alt="{{$frontend->type}}">
                                {!! Form::open(['method'=>'DELETE',
                                   'action'=>['App\Http\Controllers\FrontendController@destroyPhoto', $photo->id]]) !!}
                                ...
                                <div class="modal fade" id="exampleModal{{ $photo->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class=" modal-dialog modal-xl">
                                        <div class="modal-content">
                                            ...
                                            <img class="rounded m-2"  src="{{$photo ? asset('images/frontend') . $photo->file : 'http://placehold.it/62x62'}}" alt="{{$frontend->type}}">
                                        </div>
                                    </div>
                                </div>
                            </div>
